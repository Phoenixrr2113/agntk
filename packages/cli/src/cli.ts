...
async function main(): Promise<void> {
  ...
  if (args.outputLevel !== 'quiet') {
    ...
    // Show note when Ollama was skipped (running but no usable models)
    if (resolvedProvider.ollamaSkipReason) {
      const c = createColors(process.stderr.isTTY ?? false);
      process.stderr.write(`  ${c.yellow('note:')} ${resolvedProvider.ollamaSkipReason}\n`);
    }
  }
